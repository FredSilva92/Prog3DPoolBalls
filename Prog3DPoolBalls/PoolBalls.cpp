...
#pragma region funções da biblioteca PoolBalls
	...
	void Renderer::Read(const std::string obj_model_filepath) {
		// Implementação da leitura do arquivo .obj
		// Carregar dados do modelo, materiais e texturas para a memória do CPU
	}

	void Renderer::Send() {
		// Implementação da geração de VAO e VBO necessários
		// Enviar dados do modelo (vértices, coordenadas de textura e normais) para a memória do GPU
	}

	void Renderer::Draw(glm::vec3 position, glm::vec3 orientation) {
		// Implementação da função de renderização do modelo
		// Renderizar o modelo na posição definida por position e orientação definida por orientation
	}
	...
	void init(void) {
		...
		for (int i = 0; i < _numberOfBalls/*_numberOfBalls*/; i++) {
			textureFilenames.push_back(_ballsMaterials[i].map_kd);
		}
